...
function HeroBg({ bg, etherColor, patternVariant }){
  ...
  if (bg === "pattern"){
    ...
    return (
      <div className="hero-ether">
        <div style={{position:"absolute", inset:0, background:"radial-gradient(85% 78% at 66% 26%, rgba(14,116,144,.14), transparent 70%)"}} />
        {BGPattern && <BGPattern variant={v} mask="fade-edges" size={v==="dots"?22:46} fill="rgba(14,116,144,.2)" />}
      </div>
    );
  }
  return (
    <div className="hero-ether">
      {EtherealShadow && (
        <EtherealShadow
          color={etherColor || "rgba(30,108,140,1)"}
          grain={0.07}
          blur={24}
        />
      )}
